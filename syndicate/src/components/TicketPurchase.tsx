...
export default function TicketPurchase({ 
  ...
  const [purchaseMethod, setPurchaseMethod] = useState<'standard' | 'gasless' | 'cross-chain'>('standard');
  ...
  if (!isConnected) {
    return (
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 text-center">
        ...
        <p className="text-gray-300">Connect your wallet to purchase lottery tickets.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        <h2 className="text-2xl font-bold text-white mb-2">🎯 Purchase Lottery Tickets</h2>
        <p className="text-gray-300">
          Buy Megapot lottery tickets with multiple payment methods and cross-chain support.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-700 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Balance: {balance ? formatEther(balance.value).slice(0, 6) : '0'} {balance?.symbol}
            </p>
          </div>
      ...
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="text-sm text-gray-300">Direct purchase with gas fees</p>
            <p className="text-xs text-gray-400 mt-1">
              {availableMethods.standard ? '✅ Available' : '❌ Requires Base + balance'}
            </p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-gray-300">No gas fees with smart account</p>
            <p className="text-xs text-gray-400 mt-1">
              {availableMethods.gasless ? '✅ Available' : '❌ Requires MetaMask Flask'}
            </p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-gray-300">Buy from any supported chain</p>
            <p className="text-xs text-gray-400 mt-1">
              {availableMethods.crossChain ? '✅ Available' : '❌ Requires NEAR connection'}
            </p>
          </button>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        >
          <h3 className="text-lg font-semibold text-white">⚙️ Advanced Options</h3>
      ...
      {activeTransactions.length > 0 && (
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h3 className="text-lg font-semibold text-white mb-4">🔄 Active Transactions</h3>
              ...
              <div key={tx.id} className="bg-gray-700 rounded-lg p-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-sm px-2 py-1 rounded ${
                    ...
                    tx.status === 'executed' ? 'bg-green-600' :
                    tx.status === 'failed' ? 'bg-red-600' : 'bg-blue-600'
                  }`}>
                    {tx.status}
                  </span>
                </div>
