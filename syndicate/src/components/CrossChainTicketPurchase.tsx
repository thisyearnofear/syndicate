...
export default function CrossChainTicketPurchase({ 
  ...
  // Form state
  const [sourceChain, setSourceChain] = useState<keyof typeof SUPPORTED_CHAINS>('avalanche');
  const [targetChain, setTargetChain] = useState<keyof typeof SUPPORTED_CHAINS>('base');
  const [ticketCount, setTicketCount] = useState(1);
  const [isCreatingIntent, setIsCreatingIntent] = useState(false);
  const [isExecuting, setIsExecuting] = useState(false);

  // Intent state
  const [currentIntent, setCurrentIntent] = useState<TicketPurchaseIntent | null>(null);
  ...
  useEffect(() => {
    ...
    const loadIntents = () => {
      const intents = crossChainTicketService.getUserIntents(address);
  ...
  useEffect(() => {
    const estimateFees = async () => {
      if (ticketCount > 0) {
        ...
        try {
          const feeEstimate = await crossChainTicketService.estimateCrossChainFees({
            sourceChain,
            targetChain,
            amount: totalAmount,
          });
  ...
  const getStatusColor = (status: TicketPurchaseIntent['status']) => {
    switch (status) {
      ...
      case 'signed': return 'text-blue-400 bg-blue-900/30';
  ...
  if (!isConnected) {
    return (
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 text-center">
        ...
        <p className="text-gray-300">Connect your wallet to purchase tickets cross-chain.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        <h2 className="text-2xl font-bold text-white mb-2">🌉 Cross-Chain Ticket Purchase</h2>
        <p className="text-gray-300">
          Buy Megapot lottery tickets on Base using funds from Avalanche with NEAR chain signatures.
        </p>
      </div>
      ...
      {!currentIntent && (
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h3 className="text-lg font-semibold text-white mb-4">Create Purchase Intent</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select
                value={targetChain}
                onChange={(e) => setTargetChain(e.target.value as keyof typeof SUPPORTED_CHAINS)}
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white"
              >
            ...
            <div className="mt-6 bg-gray-700 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-300">Tickets ({ticketCount}x $1)</span>
                  <span className="text-white">${ticketCount}.00</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-300">Bridge Fee</span>
                  <span className="text-white">${formatEther(fees.bridgeFee)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-300">Gas Fee</span>
                  <span className="text-white">${formatEther(fees.gasFee)}</span>
                </div>
                <div className="border-t border-gray-600 pt-2 flex justify-between font-medium">
                  <span className="text-white">Total</span>
                  <span className="text-white">
                    ${(ticketCount + parseFloat(formatEther(fees.totalFee))).toFixed(4)}
                  </span>
                </div>
              </div>
            </div>
          ...
          {syndicateId && (
            <div className="mt-6 bg-blue-900/30 border border-blue-700 rounded-lg p-4">
              ...
              <p className="text-blue-200 text-sm">
                Syndicate: <code className="bg-blue-800 px-2 py-1 rounded">{syndicateId}</code>
              </p>
      ...
      {currentIntent && (
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h3 className="text-lg font-semibold text-white mb-4">Active Purchase Intent</h3>
          
          <div className="bg-gray-700 rounded-lg p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-white font-medium">
                {currentIntent.ticketCount} ticket{currentIntent.ticketCount > 1 ? 's' : ''}
              </span>
              <span className={`px-2 py-1 rounded text-sm ${getStatusColor(currentIntent.status)}`}>
            ...
            <div className="text-sm text-gray-300 space-y-1">
              ...
              {currentIntent.txHash && (
                <p>
                  Tx: <code className="bg-gray-600 px-1 rounded">{currentIntent.txHash.slice(0, 10)}...</code>
                </p>
              )}
              {currentIntent.errorMessage && (
                <p className="text-red-400">Error: {currentIntent.errorMessage}</p>
              )}
            </div>
          </div>
          ...
          {(currentIntent.status === 'executed' || currentIntent.status === 'failed') && (
            <button
            ...
            {userIntents.slice(0, 5).map((intent) => (
              <div key={intent.id} className="bg-gray-700 rounded-lg p-3">
                <div className="flex justify-between items-center">
                  <span className="text-white">
                    {intent.ticketCount} ticket{intent.ticketCount > 1 ? 's' : ''}
                  </span>
                  <span className={`px-2 py-1 rounded text-xs ${getStatusColor(intent.status)}`}>
