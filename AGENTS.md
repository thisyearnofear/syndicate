...
| X Layer Prize Pool Hook | Testnet + Build X AI Season entry | Live on X Layer testnet (1952). `/xlayer`: deposit/fundPot/join (write-gated), agent loop (tool registry + HITL + memory). Base remains product home. AI Season entry (closes 2026-08-21): [`docs/HACKATHON_AI_SEASON.md`](docs/HACKATHON_AI_SEASON.md). Mainnet randomness path designed (drand + permissionless relay, bonded-relay fallback) pending precompile probe + independent review — see [`docs/X_LAYER.md`](docs/X_LAYER.md) `#randomness-decision`. |
