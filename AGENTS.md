...
| Stacks x402 | Keeper implemented (testnet-first) | Resume support, error mapping, health tracking, and operator runbook shipped. Auto-purchase execution now delegates to the Stacks settlement keeper (`/api/crons/stacks-keeper`; fail-closed on `STACKS_KEEPER_ENABLED` + `STACKS_KEEPER_PRIVATE_KEY`/`STACKS_BRIDGE_OPERATOR_KEY`; float check → optional CCTP relay → classic Megapot `purchaseTickets` → receipt-verified completion; journaled to `agent_run_events`, replayed at `/api/agent/stacks/latest-run`). Custody truth corrected: USDCx peg-out settles native USDC on **Ethereum** (Circle xReserve), not Base — the keeper runs a purchase-chain float model with CCTP relay support (verified V1 addresses in `src/config/stacksKeeper.ts`; also fixed `BASE_MESSAGE_TRANSMITTER`, which previously held the TokenMessenger address). E2E on-chain proof on Base Sepolia pending a funded keeper run. **Mainnet purchase leg:** `purchaseTicketsForRecipient` now branches by chain. On 8453 it approves and calls V2 `RandomTicketBuyer.buyTickets` (the live jackpot has no classic `purchaseTickets(address,uint256,address)` selector, per the 2026-08 probes); other chains keep the classic path. Unit-tested with mocks only; no mainnet keeper run yet. `StackSettlementInput.sourceChain` lets the X Layer rail reuse the pipeline. **Known gap:** `POST /api/purchase-status` is unauthenticated. It now refuses writes to X Layer rail rows (`xlayer` / `okx-*`), but other chains' rows can still be written by anyone; a proper fix is pending. |
...
| X Layer ticket rail (OKX.AI A2MCP) | Implemented (backend + UI), not deployed — OKX Dev Day entry (closes 2026-09-25) | X Layer as a **funding rail into Base**. `POST /api/okx/tickets`: an x402 endpoint (`@okxweb3/x402-next`, 1.00 USD₮0 on chain 196, 1 ticket per call) that fails closed (503 before touching the SDK) unless `OKX_RAIL_ENABLED`, `OKX_RAIL_PAY_TO`, `OKX_RAIL_KEEPER_PRIVATE_KEY` and the OKX facilitator creds (`OKX_API_KEY`/`OKX_SECRET_KEY`/`OKX_PASSPHRASE`) are all set. Recipient defaults to the payer, decoded from the payment payload. Idempotent: `sourceTxId = okx-sha256(payment header)`, claimed in `purchase_statuses` (`source_chain 'xlayer'`). Purchase path: operator float → V2 `RandomTicketBuyer.buyTickets` → `verifyTicketPurchaseReceipt`. The SDK settles the payment only on a <400 response; the X Layer settlement tx is captured from `PAYMENT-RESPONSE` into `bridge_id` and journaled (`agent_run_events` source `xlayer-rail`, per-entry `chain`). Also shipped: free `GET /api/okx/jackpot`, `/api/agent/xlayer-rail/latest-run`, source-aware `GET /api/agent/trace`, and the `rail_xlayer` capability (`partial` behind `NEXT_PUBLIC_OKX_RAIL_ENABLED`). Code: `src/services/okxRail/`, `src/config/okxRail.ts`; tests in `tests/services/okxRail.test.ts`. UI shipped (code-verified only, not visually QA'd):
- `/ways-in` ("Ways in", overflow nav): three situation lanes with five-fact rail cards, plus the builder `#agents` section.
- Two-leg `RailReceipt` on `/purchase-status?chain=xlayer`. The page is now inside `PageShell`; states come from `deriveRailReceiptState`, which is unit-tested.
- A fourth "Agent Rail" card on `/operators`, a Fund pointer, and the home access line, whose agent clause renders only when `rail_xlayer` is `live`.

Pending: deploy, first mainnet receipt, OKX.AI listing. Plan and records: [`docs/OKX_DEV_DAY.md`](docs/OKX_DEV_DAY.md). |
| X Layer Prize Pool Hook | Testnet experiment (keeper self-play) | **Activity caveat (verified on-chain 2026-09-23):** the pool has reached epoch 522, but the only shareholder is the owner/keeper `0x9434…674f`, which also won the latest draw — the keeper funds the pot, seeds its own shares, and wins. Treat operator replays as liveness proof, not user activity. Build X AI Season closed 2026-08-21; the outcome is not recorded in the repo. No X Layer code changes since 2026-08-21. Live on X Layer testnet (1952). `/xlayer`: guided stranger walkthrough (connect → switch → faucet → shares → agent), deposit/fundPot/join (write-gated), agent loop (tool registry + HITL + memory) with a persisted session transcript (`agentSessionTranscript`) and `agent.*` observability events. A scheduled operator keeper (`/api/crons/xlayer-keeper`, daily Vercel Cron + optional GitHub Actions hourly pinger, fail-closed without `XLAYER_KEEPER_PRIVATE_KEY`, receipt-verified, full-epoch chaining per tick) persists runs to `agent_run_events`; the latest run replays publicly via `/api/agent/xlayer/latest-run`. Surfaced in nav **overflow** as "Agent Pool" (Testnet flag), never top-level, never wallet-gated. Base remains product home. AI Season entry (closed 2026-08-21): [`docs/HACKATHON_AI_SEASON.md`](docs/HACKATHON_AI_SEASON.md). Mainnet randomness path designed (drand + permissionless relay, bonded-relay fallback; EIP-2537 precompiles probe-verified on testnet 1952) pending 196 probe + independent review — see [`docs/X_LAYER.md`](docs/X_LAYER.md) `#randomness-decision`. |
| Verification provider | Noop by design | KYC is opt-in infrastructure, not the default ticket-purchase experience. |
| Operators surface (`/operators`) | Live (UI) | Worlds-cluster route unifying four operators' public run replays (Agent Pool, Season referee, Stacks settlement, X Layer Agent Rail) in a 2×2 grid via shared `OperatorRunTimeline` + `useOperatorRuns` (30s poll); receipt links route per entry `chain` (`src/config/explorers.ts`). The Agent Pool card discloses keeper self-play. Neutral accent, receipts link out, no acquisition CTA. Proof-layer story per `docs/POSITIONING.md`; nav carries the "Proof" chip. Per-purchase deep-linking: keeper journal entries carry a structured `tool_id` (source tx id) read by `GET /api/agent/stacks/trace`; `/purchase-status` embeds the per-purchase trace for Stacks flows (truthful empty state) and its no-txId landing is a paste-to-trace form; tracker links anchor in-page (`#operator-trace`), `/operators` links back to `?chain=stacks`. |
| Play home surface (`/`) | Shipped (design reset Phase 1–2) | Play-only landing: `PlayHero` (one figure, one CTA, E key), `ProofStrip`/`ReceiptStrip` proof language, `CampaignBanner` as the sole Season inset, solo `QuickPurchase` with Grow/Coordinate/Proof as quiet doors. Single mobile sticky; nav top-level ≤5 with Agent Pool overflow-only. Living-room/social-proof/mode-ladder patterns deleted (docs/DELETE.md); lab/arena token leakage blocked by the eslint register guard (docs/DESIGN.md Hard Rule 9). Funnel metrics in docs/OPERATIONS.md. |
| Season of Tickets (Tontine Pot) | Testnet loop verified; mainnet settle complete; Inco Game Jam entry | Crew-vs-crew Megapot campaign layer (tontine seats + highest-bid-wins call-the-pot auction) on real purchase/syndicate rails — no new contracts, no fabricated prizes. Registry (`seasons`, `season_crews`, `season_crew_members`, `season_call_rounds`, `season_bids`, `season_events` — migration 017), `/api/season/*` (reads public; writes rate-limited + journaled; admin `POST /api/season` fails closed without `SEASON_ADMIN_KEY`), `/season` HQ, CampaignBanner on `/` (bounded arena plate; one CTA into `/season`), time-bound Campaign nav chip + `SeasonBanner` on `/coordinate`, `season` capability in the registry (writes gated by `NEXT_PUBLIC_SEASON_WRITES_ENABLED`). Keeper cron `/api/crons/season-keeper` (keyless housekeeping — frees inactive seats, expires past-cutoff rounds; gated by `CRON_SECRET` + `SEASON_KEEPER_ENABLED`, replayed at `/api/agent/season/latest-run`). Settlement is receipt-driven: `POST /api/season/rounds/[id]/settle` verifies both real purchases on-chain (handles live mainnet V2 `TicketPurchased`/`TicketOrderProcessed`/`RandomTicketsBought`, legacy V2 `TicketPurchased`, and `UserTicketPurchase` classic/sepolia) before journaling; rejected receipts journaled as `settle.rejected`. Receipt verification routes through a dedicated Alchemy endpoint for mainnet; wide scoring scans use the public RPC (Alchemy free-tier `eth_getLogs` cap). Testnet-first on Base Sepolia (existing `MEGAPOT_BY_CHAIN` + MPUSDC config). Client-side settlement flow (`SettlePotPanel`) with `SettlementReveal` + share card, quick-crew on-chain scoring (`scoringService`, unit-tested), and Season overlay tab on `/syndicate?id=…` (`SeasonCrewOverlay` + `GET /api/season/crews?poolId=…`) are built. Round opening (`POST /api/season/crews/[id]/rounds`, Call-the-Pot UI on both `/season` and the `/syndicate` overlay) and bid membership enforcement added; API smoke test passes 25/25; end-to-end testnet purchase + receipt-verified settlement completed on Base Sepolia, with the full game loop re-verified 2026-08-14 (auction raise-only rules + anti-snipe, receipt-verified settle, tontine cut renormalization, on-chain scoring, keeper cron). Official jam rules (checked 2026-08-14) accept a working testnet prototype, so mainnet receipts are optional; a dedicated Base mainnet wallet (`0x0380…d4Ec`) completed two real `RandomTicketBuyer.buyTickets` purchases on 2026-08-14; the key was since rotated to `0x1552…83e1` (`BASE_MAINNET_WALLET_KEY`, operator RPC `BASE_MAINNET_ALCHEMY_RPC_URL`, both local-only), and key rotation is now guarded by `scripts/rotate_wallet.sh`. UX hardened after design critique (accent-aware PageShell backdrop glows, one-surface-per-stage progressive disclosure on `/season`, call/bid confirm gates, Enter-to-submit forms, invite deep link `/season?crew=` + copy button, chain-aware explorer links in `SettlementReveal`). **Arena pass shipped:** Season now has an explicit `arena` surface (warm ink/brass/copperplate hatch + display serif), deterministic crew crests, ring/grid seat table with animated cut rises, slider payout previews, a large auction stage with cutoff ring + anti-snipe announcement, Tontine lore, referee strip, and verified seal/split settlement reveal; the `/syndicate` overlay composes the same panels in a bounded arena inset. The change is visual/product UX only: existing receipt verification, real-entry scoring and write gates remain unchanged. Remaining: stranger walkthrough, demo-video capture. Canonical guide: [`docs/SEASON.md`](docs/SEASON.md). |
...
- **Other chains** are funding/routing rails into the Base-native product. Rails are either bridges (user funds arrive on Base) or **operator-float settlement** (the user pays on the source chain; a keeper buys on Base from its own float and verifies the receipt — Stacks today, X Layer planned). Float rails carry operator custody and manual rebalancing; say so wherever they are claimed.
- **Megapot** is the lottery engine; Syndicate owns coordination, yield routing, privacy, and automation.
- **X Layer** plays two separate roles: an experimental DEX-native prize-pool engine on testnet (separate from Base/Megapot), and a planned ticket rail into Base for OKX.AI agents. Do not conflate them.
