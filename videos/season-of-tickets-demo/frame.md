...
description: >
  ...
  Inter in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  # Remapped to the Syndicate app palette (src/config/design.ts): dark slate
  # canvas, amber = Megapot/play accent, violet = season/coordinate accent.
  ink-black: "#050B14"
  ink-black-alt: "#0F172A"
  fire-orange: "#FBBF24"       # app amber-400 — tickets, money, the pot
  violet: "#A78BFA"            # app violet-400 — crew / season identity
  violet-dim: "rgba(167,139,250,0.45)"
  cream: "#F8FAFC"
  cream-muted: "#94A3B8"
  cream-hint: "#64748B"
  border-dark: "rgba(255,255,255,0.10)"
  ink-on-orange-muted: "rgba(5,11,20,0.78)"
  ink-on-orange-hint: "rgba(5,11,20,0.55)"
  ink-on-orange-faint: "rgba(5,11,20,0.40)"
  ink-on-orange-border: "rgba(5,11,20,0.22)"
...
typography:
  ...
  # — display / hero ramp (Inter, lowercase, negative tracking) —
  h3:      { fontFamily: "Inter", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Inter", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Inter", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Inter", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Inter", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Inter", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Inter", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Inter", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Inter", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — Inter (display + body) and **JetBrains Mono** chrome; ignore any IBM Plex Mono reference in prose.
- **Colors** — dark slate ground (#050B14), amber (#FBBF24) is the pot/ticket accent, violet (#A78BFA) is the crew/season accent (one deliberate brand deviation from the single-accent rule).
...
text and becomes graphic primitive.** Inter `display` at 13cqw puts a single lowercase word
...
**Inter** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Inter 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Inter body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Inter `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Inter display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Inter `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Inter lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Inter `display`/`h1`
...
row of three top-border-only stat-cards — big Inter-900 numeral in `{colors.fire-orange}`, Inter
...
**Ground** ink-black. **Composes** fadelist (3 stacked Inter-900 words at 1.0/0.5/0.22), fadelist-title.
...
Inter `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Inter display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Inter display; never add a second accent color.
...
- No serif companion; chrome is never Inter.
...
- **Type** — Inter lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Inter + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
