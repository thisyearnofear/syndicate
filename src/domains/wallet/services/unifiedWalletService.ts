...
export function useUnifiedWallet(): {
    ...
    async (walletType: WalletType) => {
      ...
      if (!isBrowser()) {
        const error = createError(
          "ENV_ERROR",
          "Wallet connection is only available in browser environments",
        );
        dispatch({
        ...
        switch (walletType) {
          ...
          case WalletTypes.SOLANA:
            ...
            try {
              ...
              const solanaWallet = window.solana as
                | {
                    isPhantom?: boolean;
              ...
              if (!connection?.publicKey) {
                throw createError(
                  "CONNECTION_REJECTED",
                  "Failed to connect to Phantom. Please approve the connection.",
                );
              }
              ...
              try {
                const { solanaWalletService } = await import(
                  "@/services/solanaWalletService"
                );
              ...
              throw createError(
                "CONNECTION_FAILED",
                `Failed to connect to Phantom: ${message}`,
              );
            }
            break;
          ...
          case WalletTypes.NEAR:
            try {
              ...
              if (!accountId) {
                throw createError(
                  "CONNECTION_REJECTED",
                  "NEAR wallet connection was cancelled or timed out",
                );
              }
              ...
              if (
                ...
                throw createError(
                  "CONNECTION_TIMEOUT",
                  "NEAR wallet connection timed out. Please try again.",
                );
              }
              ...
              if (
                ...
                throw createError(
                  "CONNECTION_REJECTED",
                  "NEAR wallet connection was cancelled.",
                );
              }

              throw createError(
                "CONNECTION_FAILED",
                `Failed to connect NEAR wallet: ${message || "Unknown error"}`,
              );
            }
            break;
          ...
          case WalletTypes.STACKS:
            try {
              ...
              if (
                ...
                throw createError(
                  "CONNECTION_TIMEOUT",
                  "Stacks wallet connection timed out. Please try again.",
                );
              }
  ...
  const switchChain = useCallback(
    async (targetChainId: number | string) => {
      ...
      if (!isBrowser()) {
        throw createError(
          "ENV_ERROR",
          "Chain switching is only available in browser environments",
        );
      }
        ...
        switch (state.walletType) {
          case WalletTypes.EVM:
            ...
            if (!window.ethereum) {
              throw createError(
                "WALLET_NOT_FOUND",
                "MetaMask or WalletConnect provider not found",
              );
            }
            ...
            if (typeof targetChainId !== "number") {
              throw createError(
                "INVALID_CHAIN_ID",
                "EVM wallets require numeric chain IDs",
              );
            }
...
async function connectStacksWalletWithConnect(): Promise<{
  ...
  try {
    ...
    if (
      errorMessage?.includes("No matching provider") ||
    ...
    if (
      errorMessage?.includes("signMultipleTransactions") ||
      ...
      throw createError(
        ...
        {
          troubleshooting:
            "Make sure your wallet extension is running and unlocked.",
        },
      );
    }
