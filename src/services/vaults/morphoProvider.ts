...
export class MorphoVaultProvider implements VaultProvider {
    ...
    async getBalance(userAddress: string): Promise<VaultBalance> {
        try {
            ...
            logger.error('Failed to get Morpho vault balance', { error: error instanceof Error ? error.message : String(error) });
            return {
                deposited: '0',
                yieldAccrued: '0',
                totalBalance: '0',
    ...
    async getCurrentAPY(): Promise<number> {
        ...
        try {
            ...
            logger.error('Failed to get Morpho APY', { error: error instanceof Error ? error.message : String(error) });
            return 0;
        }
    }
