...
export class TransactionExecutor {
    ...
    async approveUsdc(ticketCount: number): Promise<string> {
        try {
            ...
            const tx = await usdcContract.approve(this.megapotAddress, approvalAmount);
    ...
    async purchaseTickets(
        ...
        try {
            if (this.baseChain.isReadOnlyMode() && recipientOverride) {
                return {
                    success: false,
                    error: 'Purchase requires NEAR Chain Signatures or manual transaction.',
                };
            }
    ...
    async purchaseTicketsWithDelegation(
        ...
        try {
            ...
            const tx = await txContract.purchaseTickets(referrer, amountUsdc, userAddress);
            const receipt = await tx.wait();
