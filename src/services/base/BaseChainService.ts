...
export class BaseChainService {
    ...
    async initializeWithWallet(chainId?: number): Promise<boolean> {
        try {
            ...
            console.error('Failed to initialize BaseChainService with wallet:', error);
            return false;
        }
    }
     ...
     */
    async initializeReadOnly(rpcUrl?: string, chainId?: number): Promise<boolean> {
        try {
            ...
            console.log(`BaseChainService initialized in read-only mode with RPC: ${this.currentRpcUrl}`);
            return true;
        } catch (error) {
            console.error('Failed to initialize read-only BaseChainService with all RPCs:', error);
            return false;
        }
    }
