...
export class DeBridgeProtocol implements BridgeProtocol {
  ...
  private quoteCache: Map<
    string,
    { quote: DeBridgeQuoteResponse; timestamp: number }
  > = new Map();
    ...
    try {
      ...
      if (cached && Date.now() - cached.timestamp < this.quoteCacheTtl) {
        ...
        return {
          fee: totalFee,
          timeMs:
            (cached.quote.order?.approximateFulfillmentDelay || 30) * 1000,
        };
      }
   ...
   */
  private async bridgeSolanaToBase(
    params: BridgeParams,
  ...
  private async getQuote(params: BridgeParams): Promise<DeBridgeQuoteResponse> {
    ...
    const srcChainToken =
      params.token && params.token.length > 10 ? params.token : USDC_SOLANA;
      ...
      try {
        ...
        if (params.options?.fallbackAddress) {
          queryParams.append(
            ...
            params.options.fallbackAddress as string,
          );
        }
        ...
        console.log(
          `[deBridge] Getting quote+tx (attempt ${attempt + 1}/${maxRetries})`,
        );
        ...
        const isInternalError =
          lastError.message.includes("INTERNAL_SERVER_ERROR") ||
          lastError.message.includes("Internal Error");

        if (attempt < maxRetries - 1) {
          console.warn(
            `[deBridge] Attempt ${attempt + 1} failed:`,
            lastError.message,
          );
          await new Promise((resolve) =>
            setTimeout(resolve, Math.pow(2, attempt) * 1000),
      ...
      try {
        ...
        if (response.status === 404) {
          ...
          if (
            ...
            data.state === "SentUnlock" ||
            data.state === "ClaimedUnlock"
          ) {
            return {
              success: true,
          ...
          if (
            ...
            data.state === "OrderCancelled" ||
            data.state === "ClaimedOrderCancel"
          ) {
            return {
              success: false,
  ...
  private createFailureResult(
    ...
    return {
      ...
      actualTimeMs,
      suggestFallback,
      fallbackReason: suggestFallback
        ? "deBridge API temporarily unavailable. Try Base-Solana Bridge."
        : undefined,
    };
  }
}
