  ...
  private async getNearAgent(_userAddress: string): Promise<AgentStatus | null> {
    try {
      ...
      return {
        ...
        name: 'The Nomad (NEAR)',
        description: 'Cross-chain automation using NEAR Chain Signatures. Gas-optimized participation routed into Base.',
        isEnabled: isEnabled,
  ...
  private async getNoLossAgent(_userAddress: string): Promise<AgentStatus | null> {
    try {
      ...
      return {
        ...
        description: 'Automated prize savings via PoolTogether v5. 100% principal protection on Base.',
        isEnabled: isEnabled,
        status: isEnabled ? 'active' : 'inactive',
        tokenSymbol: 'USDC',
        tokenAddress: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913',
  ...
  private async getScheduledAgent(_userAddress: string): Promise<AgentStatus | null> {
    ...
    try {
      ...
      return {
        ...
        description: 'Reliable recurring public-play participation via MetaMask permissions on Base.',
        isEnabled: true,
        status: 'active',
        tokenSymbol: 'USDC',
        tokenAddress: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913',
  ...
  private async getAutonomousAgent(_userAddress: string): Promise<AgentStatus | null> {
    try {
      ...
      return {
        ...
        type: 'autonomous',
        name: 'The Voyager (AI)',
        description: 'Autonomous AI participation agent using Tether WDK. Times execution based on yield performance.',
        isEnabled: isEnabled,
        status: isEnabled ? 'active' : 'inactive',
        tokenSymbol: 'USD₮',
  ...
  private async getStacksAgent(_userAddress: string): Promise<AgentStatus | null> {
    try {
      ...
      return {
        ...
        description: 'Recurring participation from Stacks using SIP-018 signatures.',
        isEnabled: true,
        status: 'active',
  ...
  private async getTonAgent(_userAddress: string): Promise<AgentStatus | null> {
    try {
      ...
      return {
        ...
        description: 'Autonomous participation agent on TON via Telegram. Uses TON/USDT flows to route into Syndicate participation.',
