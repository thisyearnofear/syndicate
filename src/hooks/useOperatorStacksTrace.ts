...
/**
 * OPERATOR STACKS TRACE — client polling for the per-purchase audit trail.
 *
 * Reads GET /api/agent/stacks/trace?sourceTxId=… (keeper journal entries
...
export function useOperatorStacksTrace(
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/agent/stacks/trace?sourceTxId=${encodeURIComponent(sourceTxId)}`,
          { cache: 'no-store' },
        );
