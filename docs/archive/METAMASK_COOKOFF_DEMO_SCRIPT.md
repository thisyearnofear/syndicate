...
**Environment**: Base **mainnet** (chain id `8453`) — real USDC, real Megapot. This is the same surface our existing users use, which is the demo's strongest signal.
**Wallet**: Regular MetaMask extension, latest version. Flask is **not** required.
**Production URL**: `https://syndicateapp.vercel.app`
**Required prod env vars** (already set on Vercel, verified live):
```
...
1. Open `https://syndicateapp.vercel.app` in a clean browser profile.
2. Open MetaMask → switch network to **Base** (chain `8453`).
3. Confirm the wallet has ≥ 5 USDC on Base mainnet and a sliver of ETH for the approve / permission txs. Budget ~$3–$6 of real USDC per take (the spend cap *is* the worst case, that's the pitch).
4. Pin one tab to `https://basescan.org` for the post-execution receipt.
5. Open the app's home page in a third tab with dev tools Network tab visible (so you can show the 1Shot JSON-RPC call in flight to `relayer.1shotapi.com`).

Smoke-check before you start the camera:
```
curl -sS https://syndicateapp.vercel.app/api/agent/autopilot/advice
# expect {"success":true,"advisor":"venice","enabled":true,"configured":true,...}
```
...
Show the Network tab: a `POST` to `https://relayer.1shotapi.com/relayers` with method `relayer_send7710Transaction` and the 7710 `permissionContext` in the body. (On mainnet Base — chain id `8453` — the service routes to the `.com` endpoint automatically; testnet would route to `.dev`.)
...
> Three things make this defensible. First, the **permission is user-revocable** at any time from the MetaMask UI — there's no agent holding a key. Second, the **cap is enforced at the protocol layer**, not just in our UI. Third, **principal is preserved by design** — the policy is bounded by expected yield, not by the user's deposit. You just watched real USDC on Base mainnet, real Megapot, real 1Shot relayer. If the agent, the relayer, and the UI all misbehave at once, the user can still only lose the cap — three dollars.
...
- Do not switch networks mid-recording — confirm you're on Base **mainnet** (8453) before the camera rolls.
- Do not record while logged into a wallet holding more than the demo budget (~$10 USDC). Use a dedicated demo wallet so a misclick can't cost more than the cap.
